...
class ModelNetHdf(Dataset):
    def __init__(self, args, root: str, subset: str = 'train', categories: List = None, transform=None):
        ...
        self.max_points = args.num_points
        ...
        with open(os.path.join(root, '{}_files.txt'.format(subset))) as fid:
            ...
            h5_filelist = [x.replace('modelnet40_ply_hdf5_2048/', '') for x in h5_filelist]
            h5_filelist = [os.path.join(self.root, f) for f in h5_filelist]
    ...
    def __getitem__(self, item):
        ...
        # if (sample['points'].shape[0] > self.max_points):
        #     idx = np.random.permutation(sample['points'].shape[0])[:self.max_points]
        #     sample['points'] = sample['points'][idx]
        #     sample['idx'] = idx
        # Apply perturbation
        ...
        rot = torch.from_numpy(sample['transform_gt'][:, :3])
        trans = torch.from_numpy(sample['transform_gt'][:, 3])
        quat = Rotation.from_matrix(rot).as_quat().astype('float32')
        pose7d_gt = np.concatenate((quat, trans.reshape(3)), axis=0)
        ...
        datas = {
            ...
            'src_normal': torch.from_numpy(sample['points_src'][:, 3:6]),
            'tgt_normal': torch.from_numpy(sample['points_ref'][:, 3:6]),
            'src_overlap': torch.from_numpy(sample['src_overlap']),  # 部分重叠点云的重叠掩码
            'tgt_overlap': torch.from_numpy(sample['ref_overlap']),
            # 'correspondences': torch.from_numpy(sample['correspondences']),  # 部分重叠点云的对应下标
            'pose7d_gt': pose7d_gt,
            'transformer': torch.from_numpy(sample['transform_gt']),
            'R': rot,
            't': trans,
            # 'idx': torch.from_numpy(sample['idx']),
            # 'corr_xyz': torch.from_numpy(corr_xyz),  # 部分重叠点云的对应坐标
        }
