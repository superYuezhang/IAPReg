...
def read_trajectory(filename, dim=4):
    ...
    with open(filename) as f:
        ...
        traj = np.asarray(traj, dtype=np.float_).reshape(-1, dim, dim)
...
def read_trajectory_info(filename, dim=6):
    ...
    cov_matrix = np.asarray(info_list, dtype=np.float_).reshape(-1, dim, dim)
...
def write_est_trajectory(gt_folder, exp_dir, tsfm_est):
    ...
    for scene_name in scene_names:
        ...
        for i in range(len(gt_pairs)):
            ...
            count += 1
        # print(count, len(gt_pairs), tsfm_est.shape)
...
def evaluate_registration(num_fragment, result, result_pairs, gt_pairs, gt, gt_info, err2=0.2):
    ...
    err2 = err2 ** 2
    gt_mask = np.zeros((num_fragment, num_fragment), dtype=np.int_)
    flags = []
...
def benchmark(est_folder, gt_folder, require_individual_errors=False):
    ...
    #                                                               np.std(precision))
    benchmark_str += "Mean recall: {:.3f}: +- {:.3f}\n".format(np.mean(recall),
                                                                  np.std(recall))
    ...
    if require_individual_errors:
        return benchmark_str, np.mean(recall), df, np.mean(re_per_scene['median']), np.mean(te_per_scene['median'])
    else:
