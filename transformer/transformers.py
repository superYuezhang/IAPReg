...
class TransformerCrossEncoder(nn.Module):
    ...
    def forward(self, src, tgt,
        ...
        if self.return_intermediate:
            return torch.stack(src_intermediate).permute(0, 2, 1, 3), torch.stack(tgt_intermediate).permute(0, 2, 1, 3)

        return src.unsqueeze(0).permute(0, 2, 1, 3), tgt.unsqueeze(0).permute(0, 2, 1, 3)
...
class TransformerSelfEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            src = layer(src, src_mask=src_mask, tgt_mask=tgt_mask,
        ...
        if self.return_intermediate:
            return torch.stack(src_intermediate).permute(0, 2, 1, 3)

        return src.unsqueeze(0).permute(0, 2, 1, 3)
...
class TransformerSelfEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.sa_val_has_pos_emb = sa_val_has_pos_emb
        self.ca_val_has_pos_emb = ca_val_has_pos_emb
        self.satt_weights = None  # For analysis
    ...
    def forward(self, src,
        ...
        if self.normalize_before:
            return self.forward_pre(src, src_mask, tgt_mask,
                                    src_key_padding_mask, src_pos)
        return self.forward_post(src, src_mask, tgt_mask,
                                    src_key_padding_mask, src_pos)
