...
class KPFEncoder(torch.nn.Module):
    def __init__(self, config, architecture=None, increase_channel_when_downsample=True):
        super().__init__()
...
class KPFDecoder(torch.nn.Module):
    def __init__(self, config, in_dim, encoder_skip_dims, architecture=None, reduce_channel_when_upsample=True):
        ...
        # Loop over consecutive blocks
        for block_i, block in enumerate(self.architecture[start_i:]):

            # Add dimension of skip connection concat
            if block_i > 0 and 'upsample' in self.architecture[start_i + block_i - 1]:
    ...
    def forward(self, pts: List[torch.Tensor]):
        ...
        for block_i, block in enumerate(self.architecture):
            ...
            if not ('pool' in block or 'strided' in block):
                layer_blocks += [block]
                if block_i < len(self.architecture) - 1 and not (
                        'upsample' in self.architecture[block_i + 1]):
                    continue
...
def compute_overlaps(batch):
    ...
    overlaps = list(batch['src_overlap']) + list(batch['tgt_overlap'])
    ...
    for p in range(1, n_pyr):
        ...
        overlap_gathered_binary = torch.where(overlap_gathered < 0.5, 0.0, 1.0)
        overlap_pyr[f'pyr_binary_{p}'] = overlap_gathered_binary

    return overlap_pyr


def compute_overlaps2(batch):
    """Compute groundtruth overlap for each point+level. Note that this is a
    approximation since
    1) it relies on the pooling indices from the preprocessing which caps the number of
       points considered
    ...
    overlaps = list(batch['src_mask_gt_list']) + list(batch['tgt_mask_gt_list'])
    kpconv_meta = batch['kpconv_meta']
    n_pyr = len(kpconv_meta['points'])
...
################
def collate_fn_descriptor(list_data, config, neighborhood_limits, architecture=None):
    batched_points_list = []
